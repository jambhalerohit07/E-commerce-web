...
const REVIEWS = [
  ...
  },
  {
    ...
    rating: 5,
    avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=60&h=60&fit=crop&crop=faces',
  },
  {
    id: 5,
  ...
  return (
    ...
    >
      <div className="wrap testi__inner">
        <h2 className="sec-title" id="testi-title">Testimonials</h2>

        {/* Quote icon — matches design */}
        <div className="testi__quote-icon" aria-hidden="true">
          <Quote size={28} />
        </div>

        {/* Review text */}
        <div className="testi__card" key={cur} aria-live="polite" aria-atomic="true">
          <p className="testi__text">{r.text}</p>

          {/* Stars */}
          <div className="testi__stars" aria-label={`${r.rating} out of 5 stars`}>
            {[1,2,3,4,5].map(s => (
              <Star key={s} size={15}
          ...
          <div className="testi__author">
            <img src={r.avatar} alt={r.author} className="testi__avatar" loading="lazy" />
            <div>
